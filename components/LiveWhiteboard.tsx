...
const LiveWhiteboard: React.FC<LiveWhiteboardProps> = ({ segments, isLive }) => {
  ...
  return (
    <div className="relative board-container w-full p-4 sm:p-6">
      <div className="flex justify-between items-center pb-4 border-b border-gray-700 mb-6">
        <h2 className="text-xl font-bold chalk-heading flex items-center">
      ...
      <div ref={whiteboardRef} className="board relative p-2 sm:p-6 rounded-md min-h-[70vh] h-full">
        {segments.length === 0 ? (
          ...
          <div className="space-y-8">
            {segments.map((segment, index) => (
              <div key={segment.id} className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center p-4 bg-transparent rounded-lg animate-fade-in">
                <div className={`chalk-text ${index % 2 === 1 ? 'md:order-2' : ''}`}>
                  <h3 className="text-xl font-semibold text-sky-200 mb-3 border-l-4 border-sky-400 pl-3 section-heading">{segment.keyIdea}</h3>
